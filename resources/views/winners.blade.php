...
<body>
    <div class="container mt-5 mb-2">
        ...
        <div class="alert alert-success text-center">
            <span>The winners are...
                @foreach(session()->get('winners') as $winner)
                {{ $winner['fullname'] }}
                @endforeach
            </span>
        </div>
        @endif
        <h5 class="text-center">Food, Fun and Stuff Raffle Application</h5>
        <div class="card mt-3">
            <div class="card-header text-center">
                Allocate Winners
            </div>
            <div class="card-body">
                <form action="{{ route('assign-winners') }}" method="POST">
                    @csrf
